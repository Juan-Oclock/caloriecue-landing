...
| `SUPABASE_SECRET_KEY` | The modern server-only secret key from the same environment's project | Production by default; approved isolated non-production only |
...
`SUPABASE_SECRET_KEY` or `SUPABASE_SERVICE_ROLE_KEY` into local Development or
Preview.
...
| Enabled environment | Supabase URL | Server-only key | Limiter secret |
|---|---|---|---|
...
applicable) and `SUPABASE_SECRET_KEY` must come from the same project. The
legacy `SUPABASE_SERVICE_ROLE_KEY` remains supported while environments migrate
to modern keys. Never reuse the Production server-only key or limiter secret in
Preview or local Development. Generate each `MACRO_CHEAT_SHEET_RATE_LIMIT_SECRET` from at
...
1. For each environment being enabled, apply and verify
   `supabase/migrations/20260810031917_macro_cheat_sheet_rate_limits.sql` in the
...
3. Configure the matching Supabase URL, that project's
   `SUPABASE_SECRET_KEY`, the environment's `RESEND_API_KEY`, and its
